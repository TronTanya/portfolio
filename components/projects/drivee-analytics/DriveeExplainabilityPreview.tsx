...
const panelGlass =
  "rounded-xl border border-white/10 bg-linear-to-b from-white/7 to-white/2 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06)] backdrop-blur-md"
  ...
  return (
    <div className={cn(panelGlass, "overflow-hidden p-4 sm:p-5", className)}>
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-[minmax(0,1.1fr)_minmax(0,0.75fr)] sm:items-stretch sm:gap-5">
        <div className="min-w-0 rounded-lg border border-white/6 bg-black/35 p-3">
        ...
        <div className="flex min-w-0 flex-col justify-between gap-3">
          <div>
            ...
            <div
              className="mt-2 flex h-24 items-end justify-between gap-1 rounded-lg border border-white/6 bg-black/25 px-2 pb-2 pt-3"
              role="img"
            ...
            >
              {chartBars.map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-sm bg-linear-to-t from-primary/25 via-accent-2/50 to-cyan-300/35"
                  style={{ height: `${Math.max(12, (h / max) * 100)}%` }}
                />
              ))}
            </div>
          </div>
          <p className="border-t border-white/8 pt-3 text-xs leading-snug text-foreground/90 sm:text-sm">
            {insight}
          </p>
        </div>
      </div>
